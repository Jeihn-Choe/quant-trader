...
class PostgreSqlMarketDataRepository:
    def __init__(
        self,
    ...
    def replace_historical_minute_bars(
        ...
        self.schema_manager.ensure_historical_minute_bar_partitions(date_from, date_to)
        with self.connection_factory.connect() as connection:
            with connection.cursor() as cursor:
                ...
                if payload:
                    insert_sql = (
                        ...
                        (symbol, symbol_name, trade_date, minute_ts, open, high, low, close, volume, source, collected_at)
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                        ON CONFLICT (symbol, trade_date, minute_ts) DO NOTHING
                        """
                    )
            ...
            connection.commit()
        return after_count if replace_existing else max(0, after_count - before_count)
    ...
    def get_loaded_historical_minute_bar_symbols(
        ...
        return self._get_loaded_symbols(
    ...
    def get_market_data_overview(self) -> MarketDataOverview:
        ...
        with self.connection_factory.connect() as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute("SELECT COUNT(*) FROM market_open_snapshot")
                market_open_snapshot_count = cursor.fetchone()[0]
                cursor.execute(
                    """
                    SELECT symbol
                    FROM (
    ...
    def list_daily_market_data_summary(
        ...
        union_filters, union_params = self._union_date_symbol_filters(symbols or [], date_from, date_to)
        with self.connection_factory.connect() as connection:
            with connection.cursor() as cursor:
                self._set_search_path(cursor)
                cursor.execute(
        ...
        for trade_date, market_open_snapshot_count, snapshot_symbol_count in snapshot_rows:
            ...
            row["symbol_count"] = max(int(row["symbol_count"]), int(snapshot_symbol_count or 0))
    ...
    def _delete_range(
        ...
        cursor.execute(f"DELETE FROM {table_name} WHERE {filters}", params)
    ...
    def _delete_range_by_source(
        ...
        filters, params = self._date_symbol_filters(symbols, date_from, date_to)
        cursor.execute(f"DELETE FROM {table_name} WHERE {filters} AND source = %s", [*params, source])
    ...
    def _count_rows_in_range(
        ...
        filters, params = self._date_symbol_filters(symbols, date_from, date_to)
        cursor.execute(f"SELECT COUNT(*) FROM {table_name} WHERE {filters}", params)
        row = cursor.fetchone()
    ...
    def _date_symbol_filters(
        ...
        filters = ["trade_date BETWEEN %s AND %s"]
            ...
            with connection.cursor() as cursor:
                ...
                if scope_key == "__ALL__":
                    ...
                    cursor.execute(
                        ...
                        WHERE dataset = %s
                          AND source = %s
                          AND (scope_key = %s OR scope_key = '__ALL__')
                          AND date_from <= %s
                          AND date_to >= %s
                        LIMIT 1
                        """,
        ...
        with self.connection_factory.connect() as connection:
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON CONFLICT (dataset, scope_key, date_from, date_to) DO UPDATE
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute(
                    ...
                    WHERE dataset = %s
                      ...
                      AND date_to >= %s
                    """,
                    [dataset, source, normalized_symbols, date_from, date_to],
                )
                rows = cursor.fetchall()
