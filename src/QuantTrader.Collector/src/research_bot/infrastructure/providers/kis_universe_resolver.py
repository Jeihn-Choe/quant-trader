...
class KisUniverseResolver:
    ...
    def refresh_masters(self) -> None:
        ...
        logger.info("KIS 종목 마스터 갱신 시작 - markets=%s", ",".join(self.markets))
        ...
        self._cached_symbol_names = None
        self._ensure_cache()
        logger.info(
            "KIS 종목 마스터 갱신 완료 - symbols=%s",
            len(self._cached_symbols or []),
        )
    ...
    def _ensure_cache(self) -> None:
        ...
        for market in self.markets:
            ...
            if source is None:
                continue
            loaded_entries = self._load_market_entries(source)
            filtered_entries = [
                (symbol, symbol_name)
                for symbol, symbol_name in loaded_entries
                if _is_numeric_stock_symbol(symbol)
            ]
            logger.info(
                "KIS 종목 마스터 로드 - market=%s total=%s filtered=%s",
                market,
                len(loaded_entries),
                len(filtered_entries),
            )
            for symbol, symbol_name in filtered_entries:
...
def _is_numeric_stock_symbol(symbol: str) -> bool:
