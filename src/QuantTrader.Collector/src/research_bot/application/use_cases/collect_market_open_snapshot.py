...
class CollectMarketOpenSnapshotUseCase:
    ...
    def execute(self, command: CollectMarketOpenSnapshotCommand) -> CollectResult:
        ...
        logger.info(
            "장 시작 스냅샷 대상 종목 확인 완료 - requested=%s resolved=%s",
            len(command.symbols),
            len(symbols),
        )
        logger.info("장 시작 스냅샷 pending 종목 확인 시작")
        ...
        logger.info("장 시작 스냅샷 pending 종목 확인 완료 - pending=%s", len(pending_symbols))
        ...
        logger.info(
            "장 시작 스냅샷 종목 단위 조회 시작 - workers=%s, symbols=%s",
            worker_count,
            total_symbols,
        )

        with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="open-snapshot-fetch") as executor:
            future_map = {
                executor.submit(
                    self.provider.get_symbol_market_open_snapshots,
                    symbol,
                    command.date_from,
                    command.date_to,
            ...
            for future in as_completed(future_map):
                ...
                try:
                    ...
                    if not rows:
                        ...
                        continue
                    _validate_market_open_snapshots(rows, [symbol], command.date_from, command.date_to)
                    symbol_written = self.repository.replace_market_open_snapshots(
                        symbols=[symbol],
                        date_from=command.date_from,
                        date_to=command.date_to,
                        rows=rows,
        ...
        if errors:
            ...
            warning_message = (
                f"장 시작 스냅샷 종목 단위 조회 중 일부 종목이 실패했습니다. {preview}{suffix}"
            )
        else:
            self.repository.mark_market_open_snapshots_loaded(
                requested_symbols=requested_symbols,
    ...
    def _resolve_pending_symbols(
        ...
        if replace_existing:
            return symbols
        loaded_symbols = self.repository.get_loaded_market_open_snapshot_symbols(
            symbols=symbols,
            date_from=date_from,
            date_to=date_to,
            source=self.provider.name,
