...
현재는 연구용 프로젝트가 실제 구현되어 있고, 실거래 쪽은 이후 확장을 위한 자리만 먼저 잡아둔 상태입니다.

## 현재 구현 범위

- `apps/research/bot/`: FastAPI, DuckDB, ORB 계산, ORB 돌파 탐지, 모의 시장데이터 적재
- `apps/research/web_ui/`: React 기반 연구용 UI, 데이터 적재 화면, ORB 스캔 화면, 결과 요약/테이블
- 한국투자증권 인증 및 실제 호출부는 빈 구현으로 남겨두고, 추후 토큰/인증정보를 넣어 연결하는 구조로 준비해뒀습니다.
